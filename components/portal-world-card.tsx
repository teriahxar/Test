...
export function PortalWorldCard({
  ...
  return (
    <article
      className={cn(
        "group surface-card rounded-[32px] p-7 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_24px_44px_rgba(83,71,56,0.12)]",
        className
      )}
    >
      <div className="mx-auto flex h-32 w-full items-center justify-center rounded-[26px] border border-white/70 bg-white/72">
        <div className="relative h-24 w-[240px]">
          <Image src={withBasePath(logoSrc)} alt={`${title} logo`} fill className="object-contain drop-shadow-[0_12px_20px_rgba(83,71,56,0.08)]" />
        </div>
      </div>
